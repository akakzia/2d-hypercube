...
class HypercubeEnv(gym.Env):
    ...
    def sample_tasks(self, num_tasks):
        ...
        elif self.env_description['generation_zone'] == 'b':
            goals = self.np_random.uniform(low=0, high=self.max_position, size=(num_tasks, self.n))
        elif self.env_description['generation_zone'] == 'c':
            goals = self.np_random.uniform(low=-self.max_position, high=0, size=(num_tasks, self.n))
        elif self.env_description['generation_zone'] == 'd':
            goals = self.np_random.uniform(low=[0, -self.max_position],  high=[self.max_position, 0],
                                           size=(num_tasks, self.n))
        elif self.env_description['generation_zone'] == 'abc':
            goals = self.np_random.uniform(low=-self.max_position, high=self.max_position, size=(num_tasks, self.n))
            for goal in goals:
                if goal[1] < 0:
                    goal[0] = -np.abs(goal[0])
        else:
            goals = self.np_random.uniform(low=-self.max_position, high=self.max_position, size=(num_tasks, self.n))
